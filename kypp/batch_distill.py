...
shows the pass/fail boundary), then consolidate. Backend = distiller_from_env (KYPP_DISTILL_MODEL);
...
def main():
    ...
    if args.plan:
        ...
        return
    if not os.environ.get("KYPP_DISTILL_MODEL"):
        print("WARNING: KYPP_DISTILL_MODEL unset — re-distilling with the heuristic (no LLM gain)",
              file=sys.stderr)
